...
class NuScenesDataset(Custom3DDataset):
    ...
    def __init__(self,
        ...
        for i in range(len(self.data_infos)):
            sample_i = nusc.get('sample', self.data_infos[i]['token'])
            scene_i = nusc.get('scene', sample_i['scene_token'])
            if 'Rain' not in scene_i['description']:
                data_infos_new.append(self.data_infos[i])


        self.data_infos = data_infos_new
        '''
    ...
    def get_data_info(self, index):
        ...
        #if self.test_mode is False: #在test set上进行测试，需要上传到nuscenes test server时，用这句判断
        if 1:
            annos = self.get_ann_info(index)
            input_dict['lidar_ann_info'] = annos
